...
int main (int argc, char *argv[]) {
  ...
  try {
    ...
    TString pdfname = "angle_difference_fit.pdf";
    c->Print(pdfname + "[", "pdf");

    Double_t momentum, sideview, topview;
    Double_t x, y, z;
    x = -350; y = -12.4; z = -183.08;
    sideview = 0; topview = 0;
    ...
    TFile *ofile = new TFile("angle_difference_fit.root", "recreate");
    ...
    for (Int_t imomentum = 0; imomentum < 30; imomentum++) {
	...
	hist_angle_difference->Fill(TMath::ATan(get_angle_difference_lateral(tangent_up, tangent_down, tangent_up)));
